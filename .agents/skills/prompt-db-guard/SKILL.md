...
この skill は、`prompts.json` を壊さずに守るための監査役。
内容を直すのではなく、危ない箇所を見つけて人間に返す。
...
8. **即時修正対象**: 外部作品・ブランド・機関・サービスの直接名が、`title`、`purpose`、`summary`、`generated_prompt`、`artifact title`、`artifact path` のいずれかに残っている。具体例は ADR 0021 を参照する。
9. **文脈名の例外**: 題材の前提を説明するためのプラットフォーム名、エンジン名、API名、ツール名は、借用ではなく文脈名として扱う。
10. **docs / skills の推論混入**: `docs/*` と `.agents/skills/*` に、未確認の仕様、未実装の機能、曖昧な一般論、推論の飛躍が残っている場合は警告する。
...
- **docs / skills**: 確認できる事実、明示された決定、実行手順だけを書く。推論は書かない。
