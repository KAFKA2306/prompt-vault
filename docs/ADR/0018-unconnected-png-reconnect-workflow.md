...
新規の生成画像は `scripts/artifacts/register_generated_artifact.py` で `artifacts/NNN_slug.png` に登録できるが、既存の未接続PNGをまとめて整理する作業には別の手順が必要だった。
...
- 新規画像の登録は `python3 scripts/artifacts/register_generated_artifact.py` を単一入口とする。
- 既存の未接続PNGを再採番して `artifacts/` と `db/prompts.json` に戻す場合は `python3 scripts/artifacts/reconnect_unconnected_pngs.py` を使う。
- 事前確認だけしたい場合は `python3 scripts/artifacts/reconnect_unconnected_pngs.py --dry-run` を使う。
...
1. 新しい画像を採用するときは `scripts/artifacts/register_generated_artifact.py` を使う。
2. 既存の未接続PNGを整理するときは `scripts/artifacts/reconnect_unconnected_pngs.py` を使う。
