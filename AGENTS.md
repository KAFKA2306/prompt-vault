...
2. **静的生成**: `python3 build.py` を実行。`dist/` はこのコマンドでのみ更新される「生成物」であり、直接編集は厳禁。
3. **ローカル検証**: `python3 app.py` で表示を確認。画像が表示されない、あるいは `画像なし` のラベルが出る場合は、`artifacts/` と JSON の接続ミスを疑う。
4. **公開検証**: `scripts/verify_pages.sh` でデプロイ後の状態をシミュレート。

---

## 自律実行と完了定義

- **完遂の定義**: `build.py` の実行、ローカルでの視覚的確認、およびバリデーション（`task validate` 等）がパスした状態を指す。
