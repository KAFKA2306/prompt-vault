...
Prompt Vaultは、プロンプト本文・再利用できる構成要素・生成画像・用途・来歴を分けて管理し、必要な組み合わせを再構成できる保管庫です。その意味を示した後で、`block`、`template`、`artifact`、Pydanticなどの技術語を使います。

READMEの入口は [`KAFKA2306/articles#34`](https://github.com/KAFKA2306/articles/issues/34) の「広い問題 → 具体例 → 技術」の編集原則を維持します。プロンプト本文、生成物、来歴を混同せず、`db/prompts.json` と `artifacts/` の正準関係を崩しません。
